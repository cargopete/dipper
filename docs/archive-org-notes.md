...
### advancedsearch.php — what dipper actually uses
...
- Paging works, up to **10,000 results**, after which you get
  ...
  the status.
- `rows=1000` is accepted. dipper defaults to 500.
...
- `fl[]` is repeated once per field. `format` is a valid field and comes back
  as an array, which is how dipper flags torrent-backed items.
...
- ~300-500 ms between requests keeps 429s away. dipper defaults to 350 ms and
...
- **Webseeds (BEP 19) are where the bytes come from.** `url-list` points at
  ...
  archive.org writes `http://` URLs for hosts that speak HTTPS, so dipper
  upgrades the scheme.
...
- The infohash is the SHA-1 of the `info` dictionary **as it appears on the
  wire**. dipper locates the byte span with a small bencode scanner rather than
