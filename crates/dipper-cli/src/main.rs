//! dipper: a BitTorrent client that uses the Internet Archive as its search
...
#[command(
    name = "dipper",
    version,
    about = "Search the Internet Archive and fetch it over BitTorrent",
    long_about = None
)]
struct Cli {
    #[command(subcommand)]
...
impl Filters {
    ...
    fn apply(&self, query: &str) -> String {
        ...
        if self.torrents_only {
            parts.push(format!(
                "format:\"{}\"",
                dipper_ia::metadata::TORRENT_FORMAT
            ));
        }
        parts.join(" AND ")
    }
...
async fn run(cli: Cli) -> Result<()> {
    match &cli.command {
        Command::Search {
            ...
            if *remote {
                ...
                if catalogue.is_empty()? {
                    bail!(
                        "the catalogue is empty. Run `dipper harvest <query>` first, or search \
                         archive.org directly with `dipper search --remote {query}`"
                    );
                }
        ...
        Command::Torrent {
            ...
            let meta = dipper_ia::Metainfo::parse(&raw)?;
...
fn print_local_hits(hits: &[dipper_index::Hit], json: bool) -> Result<()> {
    if json {
        let rows: Vec<_> = hits
            .iter()
            .map(|hit| {
...
fn print_torrent(meta: &dipper_ia::Metainfo) {
...
fn index_dir(cli: &Cli) -> Result<PathBuf> {
    ...
    let dirs = directories::ProjectDirs::from("org", "nightswatch", "dipper")
...
fn init_tracing(verbose: u8) {
    let filter = match verbose {
        0 => "dipper=info,dipper_ia=info,dipper_index=warn",
        1 => "dipper=debug,dipper_ia=debug,dipper_index=info",
        _ => "debug",
    };
...
mod tests {
    ...
    fn search_hits_become_records() {
        ...
        let page = dipper_ia::SearchPage::parse("test", body).unwrap();
