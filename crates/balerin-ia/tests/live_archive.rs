...
//!
//! ```sh
//! cargo test -p balerin-ia -- --ignored --test-threads=1
//! ```
//!
//! These are the tests that catch archive.org changing under us, which it does.

use balerin_ia::{AdvancedQuery, ClientConfig, IaClient, advanced, metadata, torrent};
...
async fn missing_items_are_reported_as_not_found() {
    ...
    assert!(matches!(err, balerin_ia::Error::ItemNotFound(_)), "{err:?}");
}
