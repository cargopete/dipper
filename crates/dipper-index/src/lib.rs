...
mod tests {
    ...
    fn round_trips_a_record() {
        let original = record("dipper-item", "The White-Throated Dipper", "texts", true);
        let catalogue = catalogue_with(std::slice::from_ref(&original));
        let stored = catalogue.get("dipper-item").unwrap().expect("stored");
    ...
    fn finds_items_by_free_text() {
        let catalogue = catalogue_with(&[
            record("a", "The White-Throated Dipper", "texts", true),
            record("b", "Kingfishers of Britain", "texts", false),
        ]);
        let hits = catalogue.search("dipper", 10).unwrap();
