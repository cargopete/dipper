...
mod tests {
    ...
    fn round_trips_a_record() {
        let original = record(
            "balerion-item",
            "The White-Throated Balerion",
            "texts",
            true,
        );
        let catalogue = catalogue_with(std::slice::from_ref(&original));
        let stored = catalogue.get("balerion-item").unwrap().expect("stored");
    ...
    fn finds_items_by_free_text() {
        let catalogue = catalogue_with(&[
            record("a", "The White-Throated Balerion", "texts", true),
            record("b", "Kingfishers of Britain", "texts", false),
        ]);
        let hits = catalogue.search("balerion", 10).unwrap();
