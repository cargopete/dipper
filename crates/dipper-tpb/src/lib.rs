...
//! [`dipper_bt`], which is handed a magnet and never asked where it came from.
//!
//! ```no_run
//! # async fn demo() -> Result<(), dipper_tpb::Error> {
//! use dipper_tpb::{TpbClient, category, search};
//!
...
//! not a licence. dipper says so in the interface rather than in a comment
//! nobody reads.
