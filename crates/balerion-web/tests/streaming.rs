...
//!
//! A local HTTP server stands in for a BEP 19 webseed, balerion downloads from
...
/// A torrent being downloaded from a local webseed, served by a live balerion.
struct Harness {
    base: String,
    hash: String,
    one: Vec<u8>,
...
async fn the_page_and_its_assets_are_served() {
    ...
    assert!(html.contains("<title>Balerion</title>"));
