...
//!
//! A local HTTP server stands in for a BEP 19 webseed, balerin downloads from
...
use balerin_bt::Strategy;
use balerin_bt::metainfo::Metainfo;
use balerin_bt::session::{self, DownloadConfig, VerifyPolicy};
use balerin_web::state::{AppState, ServeConfig, Torrent};
...
/// A torrent being downloaded from a local webseed, served by a live balerin.
struct Harness {
    base: String,
    hash: String,
    one: Vec<u8>,
...
async fn harness() -> Harness {
    ...
    let addr = listener.local_addr().unwrap();
    tokio::spawn(async move {
        let _ = axum::serve(listener, balerin_web::router(state)).await;
    });
...
async fn the_page_and_its_assets_are_served() {
    ...
    assert!(html.contains("<title>balerin</title>"));
