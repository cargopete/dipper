//! A local web UI for balerin: paste a magnet, watch the video while it
//! downloads.
//!
...
pub fn default_data_dir() -> std::path::PathBuf {
    directories::ProjectDirs::from("", "", "balerin")
        .map(|dirs| dirs.cache_dir().join("torrents"))
        .unwrap_or_else(|| std::path::PathBuf::from(".balerin"))
}
    ...
    if !config.host.is_loopback() {
        ...
        tracing::warn!(
            ...
            "serving beyond localhost. Anyone who can reach this port can make \
             balerin download things on your behalf"
        );
    }
