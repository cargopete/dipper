...
pub enum Playback {
    ...
    /// balerin does not do yet, so it is offered as a download instead.
    NeedsRemux,
...
/// offered, because balerin can convert them when ffmpeg is present. An item
...
pub fn best_to_play(files: &[balerin_bt::TorrentFile]) -> Option<usize> {
...
mod tests {
    ...
    fn file(path: &str, length: u64) -> balerin_bt::TorrentFile {
        balerin_bt::TorrentFile {
            path: path.to_string(),
            length,
            offset: 0,
        }
