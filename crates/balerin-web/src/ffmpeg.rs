...
//! the player falls back to offering a download, which is what balerin did
//! before this module existed. The single binary promise survives: transcoding
//! is an enhancement, not a requirement.
//!
//! The pleasant part of this design is that ffmpeg reads balerin's own range
...
impl Tools {
    ...
    ///
    /// `url` points back at balerin's own range endpoint, so this works on a
