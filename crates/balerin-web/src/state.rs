...
use balerin_bt::{DownloadSummary, InfoHash, Metainfo, SessionHandle};
...
pub struct Torrent {
    ...
    pub task: JoinHandle<balerin_bt::Result<DownloadSummary>>,
...
impl Torrent {
    pub fn new(
        ...
        task: JoinHandle<balerin_bt::Result<DownloadSummary>>,
        refill: JoinHandle<()>,
        root: PathBuf,
    ) -> Self {
        Self {
...
impl AppState {
    pub fn new(config: ServeConfig) -> Self {
        Self {
            config,
            ia: balerin_ia::IaClient::new().expect("the HTTP client failed to build"),
            tpb: balerin_tpb::TpbClient::new().expect("the HTTP client failed to build"),
            tools: None,
...
/// balerin acquiring a state store to go wrong.
const KEEP_MARKER: &str = ".balerin-keep";
