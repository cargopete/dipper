...
/// A file written under a different name fails this check and is reported as
/// "not a balerion resume file", which sends the caller back to re-hashing the
/// files: slower, and correct. Reading one as though the layout matched would
/// hand back somebody else's bitfield.
const MAGIC: &[u8] = b"BALERION";
const VERSION: u8 = 1;

/// Every field's position, derived from the magic rather than written out as
/// literals.
///
/// It used to be a run of hand-computed numbers, which was fine until the magic
/// changed length and every one of them was silently off by one. A parser whose
/// offsets are arithmetic on one constant cannot drift out of step with the
/// writer that shares them.
const OFF_VERSION: usize = MAGIC.len();
const OFF_INFOHASH: usize = OFF_VERSION + 1;
const OFF_PIECE_LENGTH: usize = OFF_INFOHASH + 20;
const OFF_TOTAL_LENGTH: usize = OFF_PIECE_LENGTH + 8;
const OFF_PIECE_COUNT: usize = OFF_TOTAL_LENGTH + 8;
const OFF_FLAGS: usize = OFF_PIECE_COUNT + 4;

/// Header bytes before the bitfield: magic, version, infohash, piece length,
/// total length, piece count, flags.
const HEADER_LEN: usize = OFF_FLAGS + 1;
...
impl ResumeState {
    ...
    pub fn path(root: impl AsRef<Path>, info_hash: InfoHash) -> PathBuf {
        root.as_ref()
            .join(format!(".balerion-{}.resume", info_hash.to_hex()))
    }
    ...
    pub fn decode(raw: &[u8]) -> Result<Self> {
        ...
        if &body[..MAGIC.len()] != MAGIC {
            return Err(Error::Metainfo("not a balerion resume file".into()));
        }
        if body[OFF_VERSION] != VERSION {
        ...
        info_hash.copy_from_slice(&body[OFF_INFOHASH..OFF_PIECE_LENGTH]);
        let piece_length =
            u64::from_le_bytes(body[OFF_PIECE_LENGTH..OFF_TOTAL_LENGTH].try_into().unwrap());
        let total_length =
            u64::from_le_bytes(body[OFF_TOTAL_LENGTH..OFF_PIECE_COUNT].try_into().unwrap());
        let piece_count =
            u32::from_le_bytes(body[OFF_PIECE_COUNT..OFF_FLAGS].try_into().unwrap()) as usize;
        let clean = body[OFF_FLAGS] & FLAG_CLEAN != 0;
