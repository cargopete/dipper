//! balerin: a BitTorrent client that uses the Internet Archive as its search
...
use balerin_index::{Catalogue, Record};
...
impl Filters {
    ...
    fn apply(&self, query: &str) -> String {
        ...
        if self.torrents_only {
            parts.push(format!(
                "format:\"{}\"",
                balerin_ia::metadata::TORRENT_FORMAT
            ));
        }
        parts.join(" AND ")
    }
...
async fn run(cli: Cli) -> Result<()> {
    match &cli.command {
        Command::Search {
            ...
            if *remote {
                ...
                if catalogue.is_empty()? {
                    bail!(
                        "the catalogue is empty. Run `balerin harvest <query>` first, or search \
                         archive.org directly with `balerin search --remote {query}`"
                    );
                }
        ...
        Command::Torrent {
            ...
            let meta = balerin_ia::Metainfo::parse(&raw)?;
        ...
        Command::Serve {
            ...
            let mut config = balerin_web::ServeConfig {
                host: *host,
                port: *port,
                data_dir: output.clone().unwrap_or_else(balerin_web::default_data_dir),
                max_peers: *max_peers,
                use_dht: !*no_dht,
            ...
            balerin_web::serve(config).await
        }
...
fn print_local_hits(hits: &[balerin_index::Hit], json: bool) -> Result<()> {
    if json {
        let rows: Vec<_> = hits
            .iter()
            .map(|hit| {
...
fn print_torrent(meta: &balerin_ia::Metainfo) {
...
fn index_dir(cli: &Cli) -> Result<PathBuf> {
    ...
    let dirs = directories::ProjectDirs::from("org", "nightswatch", "balerin")
...
fn init_tracing(verbose: u8) {
    let filter = match verbose {
        0 => "balerin=info,balerin_ia=info,balerin_index=warn",
        1 => "balerin=debug,balerin_ia=debug,balerin_index=info",
        _ => "debug",
    };
...
mod tests {
    ...
    fn search_hits_become_records() {
        ...
        let page = balerin_ia::SearchPage::parse("test", body).unwrap();
