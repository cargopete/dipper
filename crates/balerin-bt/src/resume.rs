...
/// Seven bytes exactly, because [`HEADER_LEN`] and the offsets below are
/// written in terms of that. "BALERIN" happens to be seven letters, so the
/// rename cost the format nothing. A file written by the old name fails this
/// check and is reported as "not a balerin resume file", which sends the caller
/// back to re-hashing the files: slower, and correct. Reading it as though the
/// layout matched would hand back somebody else's bitfield.
const MAGIC: &[u8; 7] = b"BALERIN";
const VERSION: u8 = 1;
...
impl ResumeState {
    ...
    pub fn path(root: impl AsRef<Path>, info_hash: InfoHash) -> PathBuf {
        root.as_ref()
            .join(format!(".balerin-{}.resume", info_hash.to_hex()))
    }
    ...
    pub fn decode(raw: &[u8]) -> Result<Self> {
        ...
        if &body[..7] != MAGIC {
            return Err(Error::Metainfo("not a balerin resume file".into()));
        }
        if body[7] != VERSION {
